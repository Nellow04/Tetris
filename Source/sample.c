...
3. OR ()       : Restituisce vero (1) se almeno una delle espressioni ? vera.
   Esempio: int a = 1, b = 0; int risultato = a  b;  // risultato = 1
4. XOR logico    : Non esiste un operatore diretto, ma si pu? simulare:
...
3. OR (|)        : Restituisce 1 se almeno uno dei bit ? 1.
